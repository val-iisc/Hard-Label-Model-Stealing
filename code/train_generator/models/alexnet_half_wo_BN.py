...
class AlexNet_half_wo_BN(nn.Module):
    def __init__(self, num_classes=10):
        super(AlexNet_half_wo_BN, self).__init__()
    ...
    def forward(self, x):
        layer1 = self.pad(self.lrn(self.relu(self.conv1(x))))
        layer2 = self.pad(self.lrn(self.relu(self.conv2(layer1))))
        layer3 = self.relu(self.conv3(layer2))
        layer4 = self.relu(self.conv4(layer3))
        layer5 = self.pad(self.relu(self.conv5(layer4)))
        flatten = layer5.view(-1, 64*3*3)
        fully1 = self.relu(self.fc1(flatten))
        fully1 = self.drop(fully1)
        fully2 = self.relu(self.fc2(fully1))
        fully2 = self.drop(fully2)
        logits = self.fc3(fully2)
        #softmax_val = self.soft(logits)

        return logits

model = AlexNet_half_wo_BN(10)
